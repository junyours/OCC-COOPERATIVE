				...
				<div class="page-header page-header-default">
					<div class="page-header-content">
						<div class="page-title">
							<h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Dashboard </span> -  Inventory  Report</h4>
						</div>
					</div>
				...
				<div class="content">
				    <div class="panel panel-body ">
						<form class="heading-form" id="form-sales" method="POST">
							<input type="hidden" name="submit-inventory-report">
							<ul class="breadcrumb-elements" style="float:left" >
								<li  style="padding-top: 2px;padding-right: 2px">
									<div class="input-group">
										<input   type="text" autocomplete="off" name="date" class="form-control daterange-buttons" value=" <?php if (isset($_SESSION['inv-report'])!="") {?>   <?= $_SESSION['inv-report'] ?> <?php }else{?> <?= date("m-d-Y")?> - <?= date("m-d-Y")?>  <?php }?>"> 
									</div>
								</li>
					...
					<div class="panel panel-white border-top-xlg border-top-teal-400">
						<div class="panel-heading">
							<h6 class="panel-title"><i class="icon-chart text-teal-400"></i> List of inventory report <a class="heading-elements-toggle"><i class="icon-more"></i></a></h6>
						</div>
						    ...
						    <table class="table datatable-button-html5-basic table-hover table-bordered  ">
						       ...
						       <tr>
									<?php 
                                        ...
                                        //https://stackoverflow.com/questions/18201600/sql-sum-operation-of-multiple-subqueries
										//  SELECT t.id, 
										...
										// SELECT t.*,
										//     (SELECT SUM(subtotal) as total_cost FROM tbl_sales WHERE tbl_sales.product_id = tbl_product.product_id ) as total_service_cost
										//     FROM tbl_products INNER JOIN tbl_sales ON  tbl_product.product_id = tbl_sales.product_id
										 
										$query = 
										"   SELECT p.*,
										        (SELECT SUM(quantity_order) 
                                                    FROM tbl_sales
                                                     WHERE product_id = p.product_id   )    as total_sold,

											    (SELECT balance
                                                    FROM tbl_product_history
                                                     WHERE product_id = p.product_id
													 AND hist_date BETWEEN  '$from' AND '$to'
													  ORDER BY hist_date DESC
													) as balance_qty,
													 
											    (SELECT SUM(quantity_damage) 
                                                    FROM tbl_damage
                                                    WHERE product_id = p.product_id
													AND date_damage BETWEEN  '$from' AND '$to'
													) as total_damage
			                         ...
			                         ?>
										...
										<td style="text-align: center;width: 140px"><?= $row['balance_qty'];?></td>
										<td style="text-align: center;width: 140px"><?= $row['total_sold'];?></td>
										<td style="text-align: center;width: 140px"><?= $row['total_damage'];?></td>
									</tr>
								</tr>
							<?php } ?>
							<?php if ($i==0) {?>
